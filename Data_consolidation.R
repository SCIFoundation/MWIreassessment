...
mwiB1_data <- mwi_B1 %>%
  ...
  # get mean prevalence by school
            ...
            Ascprev = mean(asc,na.rm=T),
            Hkwprev = mean(hook,na.rm=T),
            Triprev = mean(tri,na.rm=T),
            anySTHprev = mean(sth,na.rm=T)) %>% 
...
mwiB2_data <- mwi_B2 %>%
  ...
  summarise(Year = 2013, Test = "KK/UF", Visit_date=unique(na.omit(dateofsurvey)),
            ...
            anySCHprev = mean(anySCH,na.rm=T),
            Ascprev = mean(Asc,na.rm=T),
            Hkwprev = mean(Ank,na.rm=T),
            Triprev = mean(Trich,na.rm=T),
            anySTHprev = mean(STH,na.rm=T)) %>% 
...
mwiR1_data <- mwi_R1 %>%
  ...
  summarise(Year = 2017, Test = "KK/UF",Visit_date=head(Date.y,1),
            ...
            anySCHprev = mean(anySCH,na.rm=T),
            Ascprev = mean(Asc_bin,na.rm=T),
            Hkwprev = mean(Hw_bin,na.rm=T),
            Triprev = mean(Tric_bin,na.rm=T),
            anySTHprev = mean(STH,na.rm=T)) %>% 
...
mwiR3_data <- mwi_R3 %>%
  ...
  summarise(Year = 2019, Test = "KK/UF",Visit_date=unique(na.omit(date)),
            ...
            anySCHprev = mean(anySCH,na.rm=T),
            Ascprev = mean(Asc_bin,na.rm=T),
            Hkwprev = mean(Hkw_bin,na.rm=T),
            Triprev = mean(Tri_bin,na.rm=T),
            anySTHprev = mean(STH,na.rm=T)) %>% 
